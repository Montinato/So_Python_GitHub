...
class Stampatore(Thread):
    ...
    def __init__(self,SP):
        super().__init__()
        self.SP = SP
        self.conteggio = Stampatore.cont
    ...
    def run(self):
        while(True):
            ...
            print(str(Stampatore.cont))
            if(Stampatore.cont == 100):
                break
        ...
        with self.L:
            ...
            for p in range(0,self.NCODE):
                ...
                if len(self.C[p]) > 0 and self.__noAltreSoglieSuperate(p):
                    ...
                    print("Dopo questo dovrei finire\n")
                    return self.C[p].pop(0)
    
    def stop(self):
        with self.L: 
            print("ARRESTO = TRUE \n")
            print("Stampe prenotate = " + str(len(self.C[0])) +" " + str(len(self.C[1])) +" " + str(len(self.C[2])) +"\n")
            self.arresto = True
    ...
    def run(self):
        ...
        while(True):
            ...
            if(count == 100):
                ...
                # IL PROGRAMMA NON TERMINA, DEVO RISOLVERE 

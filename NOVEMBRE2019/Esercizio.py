...
class RoundRobinLockStarvationMitigation(RoundRobinLock):
    ...
    def __init__(self,N : int):
        ...
        # Mi serve per vedere chi svegliare
        self.ultimoChePrendeIlLock = 0
    ...
    def urgentAcquire(self,id : int):
        ...
        with self.lock:
            ...
            id = self.setPresident(id)
            self.conditions[id].notifyAll()
            print("Il presidente con id = " + str(id) + " acquisisce il lock \n")
            self.possessori += 1
            self.consecutiveOwners += 1
            ...
            print("Si prepara ad asssere svegliato " + str(self.ultimoChePrendeIlLock) + "\n")
            
            self.conditions[self.ultimoChePrendeIlLock].notifyAll()
...
class Animale(Thread):
    ...
    def run(self):
        while(self.iterazioni > 0):
                ...
                if(self.iterazioni == 990):
                    print("INVOCO urgentAcquire()\n")
...
# FUNZIONA MA C'E' UN PO DI STARVATION 
...
class DelayedBlockingQueue:
    ...
    def takeLast(self):
        with self.wlock:
            #print("TAKE LASTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT" + "\n")
            while len(self.coda) == 0 or len(self.scadenze) == 0:

                # if(len(self.coda) == 0 ):
                #     print("NON CI SONO ELEMENTI IN CODA" + "\n")

                
                # if(len(self.scadenze) == 0 ):
                #     print("NON CI SONO ELEMENTI IN SCADENZE" + "\n")

                # print("WAAAAAAAAAAAAAAAAAAIIIIIIIIIIIIIIITTTTTTTTTTTTTTTTTTTTT" + "\n")

                self.empty.wait()

            if len(self.scadenze) > 0:
                k = self.ultimoElemento()
                del self.scadenze[k]
    ...
    def ultimoElemento(self):
        indiceMax = None
        maxium = 0    
        for k in self.scadenze:
            if self.scadenze[k] > maxium:
                indiceMax = k
                maxium = self.scadenze[k]
        return indiceMax

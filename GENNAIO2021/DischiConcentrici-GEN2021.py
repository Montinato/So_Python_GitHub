...
class DischiConcentrici():
    ...
    def set(self, i : int, v : int, d : int):
        with self.lock:
            ...
            elif v != 0: # and self.cambioShift == True:
                self.waitCondition.notifyAll()

    # def oldGet(self,i : int,d : int):
    #     with self.lock:
    #         self.valueOldShift = self.shiftAttuale
    #         dprint("VALORE oldShift = " + str(self.valueOldShift) +"\n")
    #         while (d == 0 and self.Out[self._om(i)] == 0) or (d == 1 and self.In[i] == 0) and self.cambioShift == False:
    #             dprint("In attesa")
    #             self.waitCondition.wait()

    #         if(self.shiftAttuale == self.valueOldShift):
    #             self.cambioShift = False

    #             dprint("Risvegliato")
    #             dprint(str(self.shiftAttuale))
    #             dprint(str(self.valueOfShift))
